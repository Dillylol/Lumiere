...
| P5 | Home, blocks, paths, bindings, preview, robot status, learning, undo and recovery | Browser workflows, keyboard access and accessibility checks | Done |
| P6 | Java editor, tool installer, build/deploy and JVM runner | Build errors become editor markers; desktop quickstart smoke test | Done |
| P7 | Cross-platform installers, update signing, docs and release workflows | All release checks pass and publication gates are satisfied | 0.1.1 released; updater signing remains optional follow-up work |
...
Lumière 0.1.1 and its JitPack coordinates are published. Ask before pushing further branches, creating another repository, tagging or publishing another release, or hosting the web app. The exposed updater key must be rotated and CI secrets replaced before updates are enabled. Hardware verification remains a separate documented checklist.
